...
it('removes the subscription', function (): void {
    $this->action->execute($this->user, $this->company);

    expect($this->user->fresh()->subscribedCompanies)->toHaveCount(0);
});

it('removes job_posting_user records for the company', function (): void {
...
it('does not affect other users subscriptions', function (): void {
...
it('does not affect job postings from other companies', function (): void {
