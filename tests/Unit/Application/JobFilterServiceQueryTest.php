...
it('applies title include filter to query', function () {
...
it('applies title exclude filter to query', function () {
    $company = Company::factory()->create();
    JobPosting::factory()->create(['company_id' => $company->id, 'title' => 'Senior Engineer']);
    JobPosting::factory()->create(['company_id' => $company->id, 'title' => 'VP of Sales']);
...
it('applies remote only filter to query', function () {
    $company = Company::factory()->create();
...
it('applies department include filter to query', function () {
    $company = Company::factory()->create();
...
it('returns unmodified query when filter is null', function () {
    $company = Company::factory()->create();
